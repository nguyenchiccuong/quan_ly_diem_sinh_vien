...

    {
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string strLenh = "SELECT MA_LOP_TC From (SELECT MA_NK_HK FROM NIEN_KHOA_HOC_KY WHERE  NIEN_KHOA=" + cboNK.Text.Trim().Substring(0, 4) + "AND HOC_KY="+cboHK.Text.Trim() +") NKHK, LOP_TIN_CHI LTC WHERE MA_MH='"+cboMonHoc.SelectedValue.ToString().Trim() + "' AND NHOM="+cboNhom.Text.Trim()+ "AND LTC.MA_NK_HK=NKHK.MA_NK_HK AND  LTC.HUY='False'";
